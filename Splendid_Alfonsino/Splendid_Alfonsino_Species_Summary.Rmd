...
author: "NPFC Splendid Alfonsino Small Working Group"
date: "`r Sys.Date()`"
output:
...
```{r picture, fig.cap="**Figure 1 Photographs of Beryx splendens.**A) postlarva, B) juvenile, C) young, D) adult (from Watari et al. 2016)", echo=FALSE}

knitr::include_graphics("Figures/Splendid_Alfonsino.jpg")
...
download.file("https://www.aquamaps.org/CSV/1623909682.csv", destfile="Splendid_Alfonsino_Distribution.csv",mode="wb", quiet = FALSE)
dist_points<-read.csv("Splendid_Alfonsino_Distribution.csv",skip=7,header=TRUE)
...
png("Figures/Splendid_Alfonsino_DistributionMap.png",height=6,width=6,unit="in",res=300)
print(p)
dev.off()
```

```{r picture2, fig.cap="**Figure 2 Known distribution of Beryx splendens.**Points indicate observation data from original sources (AquaMaps 2019, October)", echo=FALSE}

knitr::include_graphics("Figures/Splendid_Alfonsino_DistributionMap.png")
```
...
p1<-ggplot(catch)+geom_bar(aes(x=Year,y=Catch,fill=Member),stat="identity",position="stack")+
  ggtitle("Splendid Alfonsino")+ylab("Catch")+xlab("") + scale_x_continuous(breaks = seq(min(catch$Year,na.rm=TRUE),max(catch$Year,na.rm=TRUE),10))+
...
png("Figures/Splendid_Alfonsino_Catch.png",width=6, height=6,units="in",res=300)
print(p1)
dev.off()
```

```{r picture3, fig.cap="**Figure 3 Catch trends of splendid alfonsino and North Pacific armorhead (Sawada et al. 2018).** The annual amounts of catch for alfonsino by each Member are shown by the bar plot. The total catch of North Pacific armorhead is also shown as a black line for reference. Note that the amounts of alfonsino and of armorhead are drawn with different scales and that misreporting was found in trawl data, which are currently being reviewed", echo=FALSE}

knitr::include_graphics("Figures/Splendid_Alfonsino_Catch.png")
...
Size composition analysis from the catch data by Japanese trawlers (Figure 4) suggests the substantial decrease in size of fish in catches over the past decade, raising the concern about recruitment overfishing.

```{r picture4, fig.cap="Figure 4 Histograms of splendid alfonsino size by year (trawl). The histograms of fork length, sampled from the catch by Japanese trawlers and measured by the scientific observers.", echo=FALSE}
...
p1<-ggplot(NPFC_effort)+geom_bar(aes(x=Year,y=Effort,fill=Member),stat="identity",position="stack")+
  ggtitle("Splendid Alfonsino")+ylab("Effort (Days fished)")+xlab("") + scale_x_continuous(breaks = seq(2001,max(NPFC_effort$Year),5))+
...
png("Figures/Splendid_Alfonsino_Effort.png",width=6, height=6,units="in",res=300)
p1
dev.off()
...
```{r picture1, fig.cap="**Figure 4. Historical fishing effort for Splendid Alfonsino.**", echo=FALSE}

knitr::include_graphics("Figures/Splendid_Alfonsino_Effort.png")

```

## Assessment

There ar no biomass estimates available for SA in NPFC waters.
...
In 2016, theinterim management measures were implemented, which includes limiting the fishing effort to the 2007's catch level, prohibiting fisheries from November to December (which corresponds to the spawning season for NPA) and not allowing fisheries in C-H Seamount and the southeastern part of Koko Seamount (for the protection of VMEs).
