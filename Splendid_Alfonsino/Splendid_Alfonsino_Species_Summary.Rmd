...
```{r picture, fig.cap="**Figure 1: Photographs of Beryx splendens.**A) postlarva, B) juvenile, C) young, D) adult (from Watari et al. 2016)", , out.width="70%", out.height="70%", fig.align='center', echo=FALSE}
...
png("Figures/SA_DistributionMap.png",height=6,width=6,unit="in",res=300)
print(p)
dev.off()
```

```{r picture2, fig.cap="**Figure 2: Known distribution of Beryx splendens.**Points indicate observation data from original sources (AquaMaps 2019, October)", out.width="70%", out.height="70%", fig.align='center', echo=FALSE}
...
png("Figures/SA_Catch.png",width=6, height=6,units="in",res=300)
print(p1)
dev.off()
```

```{r picture3, fig.cap="**Figure 3: Catch trends of splendid alfonsino over the past two decades.** The annual amounts of catch for alfonsino by each Member are shown by the bar plot.", out.width="70%", out.height="70%", fig.align='center', echo=FALSE}
...
png("Figures/SA_Effort.png",width=6, height=6,units="in",res=300)
p1
dev.off()
...
MngmtTable<-data.frame("Convention or Management Principle"=c("Biological reference point(s)", "Stock status","Catch limit","Harvest control rule", "Other"), Status=c(1,0,2,1,2),"Comment or Consideration"=c("Not established","Status determination criteria not established","Recommended catch, effort limits","Not established","No expansion of fishing beyond established areas"))

hcolor<-c("gray","red","yellow","green")

MngmtTable <- MngmtTable %>% 
  kbl(col.names=c("Convention or Management Principle","Status","Comment or Consideration"), caption = "Management Summary", escape = FALSE)%>%
  kable_classic_2(full_width=F)%>%
  column_spec(2, color = hcolor[tabledat$Status+1],
              background = hcolor[tabledat$Status+1]) %>% 
  row_spec(0, bold = T)

MngmtTable_html <-MngmtTable %>% 
  footnote(general = "<span style='color: green;'>OK</span>, <span style='color: yellow;'>Intermediate</span>, <span style='color: red;'>Not accomplished</span>, <span style='color: gray;'>Unknown</span>", escape = F)
MngmtTable_pdf <- MngmtTable %>% 
...
switch(out_type,
       html = MngmtTable_html,
       latex = MngmtTable_pdf)
```
...
data.frame(read_xlsx("SA_Species_Summary_Data.xlsx")) %>% 
  kbl(col.names = c("Data type", "Source", "Years available", "Comment"), caption = "Data Summary", ) %>% 
  kable_classic_2(full_width=F) %>% 
  row_spec(0, bold = T)
```
